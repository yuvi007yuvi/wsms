...
exports.getWeighbridgeState = getWeighbridgeState;
exports.setupWeighbridge = setupWeighbridge;
exports.runDiagnostic = runDiagnostic;
...
async function setupWeighbridge(io) {
    try {
        ...
        activePort = new serialport_1.SerialPort({
            path: portPath,
            baudRate: 9600,
            autoOpen: true
        });
    ...
    try {
        ...
        await new Promise((resolve) => {
            setTimeout(() => {
                testPort.close((err) => {
                    if (err)
                        console.error('Error closing diagnostic port:', err);
                    resolve();
                });
            }, LISTEN_DURATION);
        });
